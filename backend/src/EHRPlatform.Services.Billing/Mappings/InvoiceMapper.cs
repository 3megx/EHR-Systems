...
public class InvoiceMapper : MappingServiceBase<Invoice, InvoiceResponseDto>
{
    ...
    /// </summary>
    public EHRPlatform.Services.Billing.Features.Invoicing.Dtos.Responses.InvoiceCommandDto MapToCommandDto(Invoice invoice)
    {
        Logger.LogDebug("Mapping invoice {InvoiceId} to command DTO", invoice.Id);

        return new EHRPlatform.Services.Billing.Features.Invoicing.Dtos.Responses.InvoiceCommandDto
        {
            Id = invoice.Id,
